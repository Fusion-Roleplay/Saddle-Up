...
Citizen.CreateThread(function()
    ...
    RotateLeft()
    RotateRight()
    TriggerServerEvent("UndergroundGold:PropRequest")
end)
...
function PropPlacer(ObjectModel)
    ...
    while true do
        ...
        if PromptHasHoldModeCompleted(SetPrompt) then
                ...
                TriggerServerEvent("UndergroundGold:SaveProp", PropHash, pPos.x, pPos.y, pPos.z,
                    heading)
        ...
        if PromptHasHoldModeCompleted(CancelPrompt) then
            ...
            TriggerServerEvent('UndergroundGold:GiveAnyItem', Config.ItemNameDestilary, 1)
            end
            ObjBusy(false)
            break
        end
...
RegisterNetEvent("UndergroundGold:PlaceProp")
AddEventHandler("UndergroundGold:PlaceProp", function(object)
    print(object)
    local tempOk = CheckTemperature()
    local locNotOk = CheckLocation()
    if tempOk ~= true then
        TheoUgNotify(Config.DialogTitle, Config.BadTemp, true, 2000)
        TriggerServerEvent('UndergroundGold:GiveAnyItem', Config.Gold.itemname, 1)
...
RegisterNetEvent("UndergroundGold:RemoveProp")
AddEventHandler("UndergroundGold:RemoveProp", function()
    StopPlace = true
    for k, v in pairs(Props) do
        ...
        if distance < 2 and v.obj ~= nil and DoesEntityExist(v.obj) then
            DeleteEntity(v.obj)
            TriggerServerEvent("UndergroundGold:DeleteProp", k)
            Props[k] = nil
        end
    end
end)

RegisterNetEvent("UndergroundGold:RemoveDestillaryAndBucket")
AddEventHandler("UndergroundGold:RemoveDestillaryAndBucket", function(coords)
    ...
    TriggerServerEvent("UndergroundGold:SaveProp", DestroyedObject, destroyedCoords.x, destroyedCoords.y, destroyedCoords.z, 0)
    StartFxSmoke(DestroyedObject, destroyedCoords)
    local WaitTime = (Config.SmokeTimeDestroyed * 1000)
    Wait(WaitTime)
    TriggerServerEvent('UndergroundGold:SmokeManage', "stop", destroyedCoords, WaitTime)
end)
...
RegisterNetEvent("UndergroundGold:GetProps")
AddEventHandler("UndergroundGold:GetProps", function(data)
    for k, v in pairs(Props) do
        DeleteEntity(v.obj)
    end
    Props = data
